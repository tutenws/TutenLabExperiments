...
%
    ...
    properties (Access = private)
        ...
        lastSecs;       % last results of GetSecs
    ...
    methods
        ...
        function [action, time, timeString] = read(obj)
            ...
            % look at axes(3), which seems to go negative for right lower
            % trigger and positive for left lower trigger
            if axes(3) < 0
                buttons(9) = 1;
            elseif axes(3) > 0
                buttons(10) = 1;
            ...
            obj.buttonX = buttons(3);
            obj.buttonA = buttons(1);
            obj.buttonB = buttons(2);
            obj.buttonY = buttons(4);
            
            % State of trigger buttons
            obj.buttonLeftUpperTrigger  = buttons(5);
            obj.buttonRightUpperTrigger = buttons(6);
            % see edit above, state seems to be indicated in the axes variable
            obj.buttonRightLowerTrigger = buttons(9);
            obj.buttonLeftLowerTrigger  = buttons(10);

            
            % State of buttons labeled 'Start' and 'Back'
            obj.buttonBack              = buttons(7);
            obj.buttonStart             = buttons(8);

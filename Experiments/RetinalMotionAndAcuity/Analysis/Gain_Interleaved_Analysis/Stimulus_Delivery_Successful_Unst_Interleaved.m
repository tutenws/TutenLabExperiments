%%%%%
% For INTERLEAVED UNSTABILIZED trials only
% AB Script for categorizing if the tumbling E stimulus was delivered successfully
% in the AO system. Sometimes the system drops frames.
% Outputs are countSuccess (0 = not successful delivery)
...
% startPath = '/Users/alisabraun/Library/CloudStorage/Box-Box/Projects/RetinalMotionAndAcuity/FVM_Motion_Acuity/Data/Raw Data/Timing Data';
startPath = '/Volumes/LaCie/Retinal_MotionandAcuity_Backup_June_30/Rochester/Interleaved Gain';
...
% load exp params to filter data by gain condition
load(dataFile(end).name); % load data

% cycle through ELocs files in the folder

%for some reason, there are 200 efiles. we want 100 of them, so make the
%first 100 w/ . at the beginning go away
count = 1;
for fileNum = 1:length(eFiles)
    if eFiles(fileNum).name(1) == '.'
        count = count +1;
    end
end

eFiles = eFiles(count:length(eFiles));

for fileNum = 1:length(eFiles)
    if gainSequence(fileNum) == 0
        ...
        %end
        % if the gain of a particular trial is 0, then set everything = to NaN
    else
        deliveryCorrect(fileNum).unstxloc = NaN;
        deliveryCorrect(fileNum).unstyloc = NaN;
        deliveryCorrect(fileNum).updatedCorrect = NaN;
...
%change the gain of this ouput
expParameters.stimulusTrackingGain = 0;

%analysisDir = ['/Users/alisabraun/Library/CloudStorage/Box-Box/Projects/RetinalMotionAndAcuity/FVM_Motion_Acuity/Analysis/Matlab Analysis/Unst/' expParameters.subjectID];
analysisDir = ['/Volumes/LaCie/Retinal_MotionandAcuity_Backup_June_30/Rochester/Interleaved Gain/Code/Matlab_Analysis/' expParameters.subjectID];
...
save([stabilizedDir filesep linkFile '_DeliverySuccessGain0.mat'], 'deliveryCorrect');
save([analysisDir filesep linkFile '_DeliverySuccessGain0.mat'], 'deliveryCorrect');
save ([analysisDir filesep linkFile '_acuityDataGain0.mat'], 'expParameters', 'correctVector', 'offsetVector', 'responseVector', 'testSequence');
